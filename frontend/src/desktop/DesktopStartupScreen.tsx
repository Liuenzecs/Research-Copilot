...
export default function DesktopStartupScreen() {
  ...
  async function runAction(action: string, task: () => Promise<unknown>, successMessage: string) {
    setBusy(action);
    setError("");
    setNotice("");
    try {
      ...
      <div className="desktop-startup-panel">
        ...
        <div style={{ display: "grid", gap: 10 }}>
          ...
          <p className="desktop-startup-text">
            现在会先打开启动页，再在后台拉起本地 FastAPI sidecar。这样即使后端还没完全就绪，你也能看到当前阶段、错误信息和恢复入口。
          </p>
        </div>
        ...
        <StatusStack
          items={[
            ...(runtimeConfig.backend_error ? [{ variant: "error" as const, message: runtimeConfig.backend_error }] : []),
