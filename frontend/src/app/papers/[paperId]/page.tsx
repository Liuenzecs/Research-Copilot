...
export default async function PaperReaderPage({
  ...
  searchParams?: Promise<{ summary_id?: string | string[]; paragraph_id?: string | string[]; project_id?: string | string[] }>;
}) {
  ...
  const projectId = parseNumber(resolvedSearchParams?.project_id ?? null);
  ...
  return (
    <PaperReaderScreen
      paperId={paperId}
      requestedSummaryId={summaryId}
      requestedParagraphId={paragraphId}
      projectId={projectId}
    />
  );
}
